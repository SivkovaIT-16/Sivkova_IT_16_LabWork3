...
### Алгоритм решения
Класс Addition реализован с использованием полиморфизма через интерфейс SummVal. Интерфейс SummVal определяет общее правило для всех числовых типов, требующее реализации методов преобразования в double.  

Классы для реализации интерфейса:
1. FractionVal - для работы с дробями. Инкапсулирует объект Fraction. Метод toDouble() - преобразует дробь в вещественное число через деление числителя на знаменатель. Метод toString() - возвращает строковое представление дроби.
2.  IntegerVal - для целых чисел. Метод toDouble() - простое преобразование из int в double. Метод toString() - преобразование числа в строку.
3.  DoubleVal - для вещественных чисел. Метод toDouble() - возвращает значение как есть. Метод toString() - преобразование double в строку.

Метод sum() основан на принципе полиморфизма, так как компилятор не знает, какой из методов toDouble() будет вызван. Реализованы методы createFraction() - создает FractionVal, createInteger() - создает IntegerVal, createDouble() - создает DoubleVal.

#### Тестирование
Вывод:
1. 2 + 3/5 + 2.3 = 4.9
2. 3.6 + 49/12 + 3 + 3/2 = 12.183333333333334
3. 1/3 + 1 = 1.3333333333333333

# Задание 6
Полиморфное сравнение: equals.
...
### Алгоритм решения
Класс ComparisonCities представляет сущность города с возможностью сравнения объектов по набору путей. Поля класса: title для хранения названия города и ways - карту дорог к другим городам в формате "город-назначение: стоимость". Для доступа к полям реализованы свойства: геттер getTitle() возвращает название города, а getWays() создает защищенную копию карты дорог. Сеттеры setTitle() и setWays() выполняют валидацию входных данных перед установкой значений. Конструктор по умолчанию инициализирует город с названием "A" и пустой картой дорог. Конструктор с параметром названия проверяет корректность названия через Validator.validateTitle() и создает город с указанным именем. Конструктор с готовыми путями дополнительно валидирует карту дорог через Validator.validateWays() и проверяет отсутствие дороги в себя через containsKey(title), после чего создает копию переданной карты. Конструктор копирования создает полную копию существующего объекта. Метод addWay() реализует добавление новой дороги: сначала проверяет название города-назначения и стоимости, затем проверяется в отсутствие дублирующей дороги и что дорога не ведет в тот же город. При успешной валидации дорога добавляется в карту. Метод removeWay() выполняет удаление существующей дороги после проверки ее наличия в карте.  Для строкового представления реализован метод toString(), который формирует описание города и его дорог. В класс добавлено переопределение метода equals() для полиморфного сравнения - два города считаются одинаковыми, если у них идентичный набор путей, причем сравнение работает между объектами классов ComparisonCities, TwoWayRoad и Roads. Метод hashCode() согласован с equals() и вычисляется на основе карты дорог.

В классе Validator реализованы статические методы:
1. validateTitle() проверяет непустое название;
2. validateCost() - положительную стоимость;
3. validateWays() - корректность всех элементов карты дорог.

Используются следующие типы исключений:
1. IllegalArgumentException - некорректные параметры;
2. IllegalStateException - неподходящее состояние объекта.

В основной программе продемонстрирована работа класса:
1. Создание карты путей;
2. Создание городов с готовой картой путей;
3. Полиморфное сравнение объектов различных классов;
4. Вывод хэш-кодов
5. Изменение карты путей для повторного сравнения.
...
### Алгоритм решения
Созданы пакеты с именами вида ru.sivkova.тип. Каждый пакет содержит логически связанные классы. 

Пакет ru.sivkova.cities:
1. ComparisonCities
2. CreateCities
3. Roads
4. Route
...
Пакет ru.sivkova.fractions:
1. ForbiddenFraction
2. Fraction
3. FractionIsNumber

Пакет ru.sivkova.validator:
1. Validator

В каждом файле .java добавлены соответсвующие объявления package, обновлены импорты между пакетами.

## Задача 2
### Текст задачи
Главный метод.
Создайте пакет ru.surname.main (вместо surname необходимо подставить собственную фамилию) в котором расположить класс с точкой входа в исполнение программы (public static void main). Также следует проверить, что ни в одном другом пакете нет классов имеющих точку входа в исполнение программы. В этом же пакете необходимо расположить класс (или интерфейс) с методами из задач блока 2.5 и продемонстрировать их работоспособность (сумма комбинаций объектов разных типов).

### Алгоритм решения
Создан пакет ru.sivkova.main, в который перемещён класс main - класс с точкой входа в исполнения программы (т.к. содержится метод public static void main(String[] args)). Также в созданный пакет перемещён класс Addition из блока 2.5. Продемонстрирована работа класса Addition.
...
### Алгоритм решения
В классе Main реализован публичный статический метод power, который обрабатывает два параметра строкового типа. Сначала выполняется преобразования строк в числа с помощью метода Integer.parseInt (если на вход подана некорректная строка, выбросится исключение NumberFormatException). Далее с помощью метода Math.pow первое число возводится в степень, равную второму числу, и возвращается результат.

В основной программе продемонстрирована работа класса:
1. Выполнена проверка количества параметров
2. Если параметров достаточно:
- Вызов метода power
- Вывод результата
3. Если параметров недостаточно:
- Вывод сообщения об ошибке

Для того чтобы указать параметры необходимо в конфигурации "Main" (Верхнее меню -> "Ran" -> "Edit Configurations") в поле "Program arguments" ввести нужные аргументы.
...
- 3 в степени 4 = 81.0
2. Ошибки:
- Ошибка: Укажите два параметра.
- Ошибка: Неверный формат чисел.

# Задание 8
Пакетная инкапсуляция: package-private, protected.
...
### Алгоритм решения
Класс Point (из пакета ru.sivkova.geometry) описывает сущность, которая представляет точку на плоскости с координатами (x, y). Объявлены приватные поля класса x и y вещественного типа. Реализованы методы доступа: getX(), getY() - получение значений координат; setX(double x), setY(double y) - установка новых значений. координатКонструктор с параметрами (public) создаёт точку с указанными координатами. Конструктор копирования (package-private - доступен только в пакете ru.sivkova.geometry) создаёт копию существующего объекта Point. 

Переопределены следующие методы:
- Метод toString() - строковое представление точки в виде {x;y};
- Метод equals(Object object) - сравнение точек по координатам (возвращает true, если координаты совпадают);
- Метод hashCode() - вычисление хэш-кода;
- Метод clone() - protected(доступен только в пакете ru.sivkova.geometry) - создание копии точки через конструктор копирования.

Для демонстрации работы в пакете ru.sivkova.geometry создан класс PointTest:
1. Создание точки point1 с координатами (-8.1, 3.0);
2. Клонирование точки создание clone как копии point1;
3. Создание новой точки point2 с координатами (2, 9);
4. Вывод объектов на экран;
5. Сравнение объектов с помощью equals();
6. Вывод хэш-кодов.

#### Тестирование
Вывод:
1. Объект point1: {-8.1;3.0}
2. Объект clone: {-8.1;3.0}
3. Объект point2: {2.0;9.0}
4. point1 equals clone: true
5. point2 equals clone: false
6. Хэш-код point1: -1353382975
7. Хэш-код clone: -1353382975
8. Хэш-код point2: 2229185
